    ...
    this.product
        ...
        res => {
            ...
            for (let i = 0; i < this.products.length; i++) {
                if (this.products[i].category !=="boissons" || this.products[i].type !=="water") {
...
.then(data=>{
  ...
  firebase.storage().ref(`product/boissons/water/${filename}.jpg`).putString(this.photo,'data_url').then(img=>{this.mine=img.downloadURL})  
})
